...
def function( date = None):
    ...
    try :
        for edition in EditionCompetition.objects.filter(is_finished = False).order_by("-edition__name"):
            ...
            if last is None or (last.date + timedelta(days = 2) <= date.date() >= edition.start_date):
...
def function2( date = None):
    ...
    try :
        for edition in EditionCompetition.objects.filter(start_date__lte = date.date(), finish_date__gte = date.date()).order_by("-edition__name"):
            last = edition.edition_rankings.filter(date__range = [date.date() - timedelta(days = 3), date.date()+ timedelta(days = 3)]).count()
