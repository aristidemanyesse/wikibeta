...
class EditionCompetition(BaseModel):
    ...
    def pays(self):
        return str(self.competition.pays)
    
    def total(self):
        return str(self.edition_du_match.filter().count())
                ...
                if len(matchs) > 0:
                    for match in matchs:
                        try:
                            ...
                            print("Error ranking get_result", e)
                                                    
                    pts = win*3 + draw
                    ppg = round(pts / len(matchs), 2)
                    avg_gs = round(gs / len(matchs), 2) if gs > 0 else 0
                    avg_ga = round(ga / len(matchs), 2) if ga > 0 else 0
                    ...
                    element = {
                        ...
                        "avg_gs"  : avg_gs,
                        "avg_ga"  : avg_ga,
                        "p1_5"    : int(round(team.plus_but(1.5, date)/len(matchs), 2)*100),
                        "p2_5"    : int(round(team.plus_but(2.5, date)/len(matchs), 2)*100),
                        "m3_5"    : int(round(team.moins_but(3.5, date)/len(matchs), 2)*100),
                    }
    ...
    def avg_buts(self):
        ...
        return round(total / (len(matchs) - test), 2) if (len(matchs) - test) > 0 else 0
